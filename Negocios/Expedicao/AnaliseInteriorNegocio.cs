...
{
    public  class AnaliseInteriorNegocio
    {
        ...
        //Pesquisa por data 
        public AnaliseInteriorCollection PesqAnaliseInterior(string dataInicial, string dataFinal)
        {
            try
            {
                AnaliseInteriorCollection analiseInteriorCollection = new AnaliseInteriorCollection();
...

                {
                    ...
                    AnaliseInterior analiseInterior = new AnaliseInterior();


                    //Percorre a tabela e preenche as linhas da gride
                    if (linha["mani_data"] != DBNull.Value)
                    {   
                        analiseInterior.maniData = Convert.ToDateTime(linha["mani_data"]);
                    }

                    if (linha["mani_codigo"] != DBNull.Value)
                    {
                        analiseInterior.maniCodigo = Convert.ToInt32(linha["mani_codigo"]);
                    }

                    if (linha["Placa"] != DBNull.Value)
                    {
                        analiseInterior.Placa = Convert.ToString(linha["Placa"]);
                    }

                    if (linha["Apelido"] != DBNull.Value)
                    {
                        analiseInterior.Apelido = Convert.ToString(linha["Apelido"]);
                    }

                    if (linha["Cidade"] != DBNull.Value)
                    {
                        analiseInterior.Cidade = Convert.ToString(linha["Cidade"]);
                    }

                    if (linha["Peso"] != DBNull.Value)
                    {
                        analiseInterior.Peso = (double)Convert.ToDouble(linha["Peso"]);
                    }

                    if (linha["Valor"] != DBNull.Value)
                    {
                        analiseInterior.Valor = (double)Convert.ToDecimal(linha["Valor"]);
                    }

                    if (linha["Cliente"] != DBNull.Value)
                    {
                        analiseInterior.Cliente = Convert.ToInt32(linha["Cliente"]);
                    }

                    if (linha["pediCodigo"] != DBNull.Value)
                    {
                        analiseInterior.pediCodigo = Convert.ToInt32(linha["pediCodigo"]);
                    }

                    //Adiciona os valores encontrados a coleção
                    analiseInteriorCollection.Add(analiseInterior);
                }

                //Retorna a coleção de cadastro encontrada
                return analiseInteriorCollection;
            }
            catch (Exception ex)
            {
